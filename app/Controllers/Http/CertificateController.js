...
const SearchEngine = use('App/Common/SearchEngine')
...
class CertificateController {
    async generate({ request, response, auth }) {
        const res = await SearchEngine.validateArgs(request.all())
        if(!res.status) {
            return res.data
        }
        else {
            const { identity, email } = request.all()
    ...
    async getCertificate({ request, response }) {
        const res = await SearchEngine.queryEngine(Certificate, request.all())
